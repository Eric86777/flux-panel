...
install_caddy_from_apt() {
    ...
    if [[ $status -eq 0 ]]; then
        apt update >>"$log_file" 2>&1 || status=$?
    fi
    if [[ $status -eq 0 ]]; then
        apt install -y debian-keyring debian-archive-keyring apt-transport-https curl gnupg >>"$log_file" 2>&1 || status=$?
    fi
    ...
    if [[ $status -eq 0 ]]; then
        apt install -y caddy >>"$log_file" 2>&1 || status=$?
    fi
...
if ! command -v caddy &>/dev/null; then
    echo "🔧 安装Caddy..."
    apt update
    apt install -y curl unzip gnupg libcap2-bin
